...
retkiapuri_env = os.getenv('RETKIAPURI_ENVIRONMENT')
...
class DevConfig(Config):
    ...
    DEBUG = True
    SQLALCHEMY_DATABASE_URI = os.getenv(f'RETKIAPURI_DATABASE_URL_DEV', 'sqlite:///')

class StageConfig(Config):
    # Stage config with debugging enabled and using stage database
    DEBUG = True
    SQLALCHEMY_DATABASE_URI = os.getenv(f'RETKIAPURI_DATABASE_URL_STAGE', 'sqlite:///')